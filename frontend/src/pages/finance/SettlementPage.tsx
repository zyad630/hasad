...
export default function SettlementPage() {
  ...
  const handleCalculate = async (shipmentId: string) => {
    ...
    try {
      ...
      alert(err.data?.error || 'حدث خطأ في طلب حساب التصفية');
    }
  };
  ...
  const handleConfirm = async () => {
    ...
    try {
      await confirm({ shipment_id: previewData.shipment_id }).unwrap();
      alert('تم تأكيد التصفية وإغلاق الإرسالية بنجاح ✅');
      setPreviewData(null);
      setSelectedShipment('');
    } catch (err: any) {
      alert(err.data?.error || 'حدث خطأ أثناء التأكيد');
    }
  };
                ...
                >
                  ...
                  {openShipments.map((s: any) => (
                    <option key={s.id} value={s.id}>
                      #{s.id.substring(0,8)} | المورد: {s.supplier_name} - بتاريخ {s.shipment_date}
                    </option>
                  ))}
                </select>
                ...
                <table className="w-full text-right border-collapse">
                  <thead>
                    <tr className="bg-surface-container-low/50 text-on-surface-variant font-cairo border-b border-zinc-200">
                      ...
                      <th className="px-6 py-4 font-bold text-sm text-secondary">الصافي للمورد</th>
                  ...
                  <tbody className="divide-y divide-surface-container-high/50">
                    {settlements.map((s: any) => (
                      <tr key={s.id} className="hover:bg-emerald-50/20 transition-colors group">
                        ...
                        <td className="px-6 py-5 font-bold">{parseFloat(s.total_sales).toLocaleString()} <span className="text-xs font-normal">ج</span></td>
                        <td className="px-6 py-5 font-black text-secondary">{parseFloat(s.net_supplier).toLocaleString()} <span className="text-xs font-normal">ج</span></td>
                ...
                <>
                  ...
                  <div className="space-y-6 mb-8">
                    <div className="flex justify-between items-center group bg-primary-fixed/20 p-4 rounded-xl">
                      ...
                      <span className="text-2xl font-black text-primary">{parseFloat(previewData.total_sales).toLocaleString()} <span className="text-sm font-bold">ج.م</span></span>
                    </div>
                    ...
                    <div className="pt-6 border-t-[3px] border-dashed border-zinc-200">
                      <div className="flex justify-between items-end">
                        <div>
                          <span className="block text-xs font-black text-zinc-500 uppercase tracking-widest mb-1">الصافي النهائي المستحق للمورد</span>
                          ...
                          <span className="text-base font-bold text-emerald-600 me-2">ج.م</span>
                        </div>
        ...
        <div className="print-only fixed inset-0 bg-white z-[999] p-8 w-full max-w-4xl mx-auto" dir="rtl" style={{fontFamily: 'Cairo, sans-serif'}}>
          <div className="flex items-center justify-between border-b-4 border-emerald-900 pb-6 mb-8">
            <div>
               ...
               <p className="text-zinc-500 font-bold mt-2 text-lg">نظام الحِسبة الإلكتروني المتكامل</p>
            </div>
          ...
          <div className="bg-zinc-50 p-6 rounded-2xl mb-8 flex justify-between border border-zinc-200">
            <div>
               <p className="text-sm text-zinc-500 font-bold mb-1">السـادة / الموردين:</p>
            ...
            <div className="text-left">
               ...
               <h2 className="text-3xl font-bold font-code">{parseFloat(previewData.total_sales).toFixed(2)} ج.م</h2>
            </div>
          </div>
          ...
          <table className="w-full text-right border-collapse mb-10 border border-zinc-300 rounded-xl overflow-hidden text-lg">
            <thead>
              <tr className="bg-emerald-900 text-white">
                ...
                <th className="px-6 py-4 border border-zinc-300 font-bold text-center">المبلغ المستقطع (جنيه)</th>
              </tr>
            </thead>
            <tbody>
              <tr>
              ...
              <tr className="bg-emerald-50">
                <td className="px-6 py-6 border border-zinc-300 font-black text-xl text-emerald-900 text-center">✨ الصافي المستحق للمورد ✨</td>
                <td className="px-6 py-6 border border-zinc-300 font-black text-2xl text-center text-emerald-900 bg-emerald-100 font-code">{parseFloat(previewData.net_supplier).toFixed(2)} ج.م</td>
              </tr>
            </tbody>
          </table>
          
          <div className="grid grid-cols-2 gap-20 text-center mt-20 pt-10 px-10 text-xl font-bold">
            <div className="border-t-2 border-zinc-300 pt-4 text-emerald-900">إدارة الحِسبة (المالية)</div>
          ...
          <div className="text-center text-zinc-400 mt-16 text-sm font-bold border-t border-zinc-100 pt-4">
             تم إصدار هذا الكشف آلياً ولا يُعتد به إلا بوجود التواقيع والأختام الرسمية - Al-Hisbah SaaS
          </div>
        </div>
      )}
    </>
